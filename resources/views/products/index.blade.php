...
$lowStock = Product::where('stock', '>', 0)->whereRaw('stock <= COALESCE(min_stock_level, 10)')->count();
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="flex justify-between items-center">
                <div>
                    ...
                    <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">Kelola produk dan stok barang</p>
                </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                ...
                <a href="{{ route('products.index') }}?stock_status=low" class="block bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg hover:shadow-md transition-all duration-200 border-l-4 border-yellow-500
                    {{ request('stock_status') == 'low' ? 'ring-2 ring-yellow-500 dark:ring-yellow-400' : '' }}">
                    <div class="p-6">
                        <div class="flex items-center">
            ...
            @if(request('stock_status') == 'low' || request('stock_status') == 'out' || request('stock_status') == 'in')
                    ...
                    <div class="flex items-center justify-between">
                        <div class="flex items-center gap-2">
                            ...
                            <span class="text-sm font-medium text-yellow-800 dark:text-yellow-200">
                                Filter aktif: 
                                @if(request('stock_status') == 'low')
                                    Low Stock (stok ≤ min level)
                                @elseif(request('stock_status') == 'out')
                                    Out of Stock (stok = 0)
                                @elseif(request('stock_status') == 'in')
                                    In Stock (stok > min level)
                                @endif
                            </span>
                        </div>
                        <a href="{{ route('products.index') }}" class="text-sm text-yellow-600 hover:text-yellow-800 dark:text-yellow-400 dark:hover:text-yellow-300 flex items-center gap-1">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <form method="GET" action="{{ route('products.index') }}" class="flex flex-wrap gap-4 items-end">
                        <div class="flex-1 min-w-[200px]">
                            ...
                            <div class="relative">
                                ...
                                <input type="text" name="search" id="search" value="{{ request('search') }}" placeholder="Cari nama atau SKU..." 
                        ...
                        <div class="w-40">
                            ...
                            <select name="category" id="category" class="block w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm">
                        ...
                        <div class="w-40">
                            ...
                            <select name="stock_status" id="stock_status" class="block w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm">
                                <option value="">All</option>
                        ...
                        <div class="w-32">
                            ...
                            <select name="status" id="status" class="block w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm">
                                <option value="">All</option>
                    ...
                    <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                        <thead class="bg-gray-50 dark:bg-gray-700">
                            <tr>
                                ...
                                <th class="px-6 py-3 text-center text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">Min Level</th>
                                ...
                                <tr class="hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-center">
                                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium 
                                            {{ $product->stock <= 0 ? 'bg-red-100 text-red-800 dark:bg-red-900 dark:text-red-300' : 
                                               ($product->stock <= $product->min_stock_level ? 'bg-yellow-100 text-yellow-800 dark:bg-yellow-900 dark:text-yellow-300' : 'bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-300') }}">
                                            {{ $product->stock }}
                                        </span>
                                    </td>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                        <div class="flex items-center justify-end space-x-2">
                                            <a href="{{ route('products.show', $product) }}" class="text-blue-600 hover:text-blue-900 dark:text-blue-400 dark:hover:text-blue-300 p-1 rounded hover:bg-blue-50 dark:hover:bg-blue-900/20" title="View">
                                                <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" />
                                                </svg>
                                            </a>
                                            <a href="{{ route('products.edit', $product) }}" class="text-indigo-600 hover:text-indigo-900 dark:text-indigo-400 dark:hover:text-indigo-300 p-1 rounded hover:bg-indigo-50 dark:hover:bg-indigo-900/20" title="Edit">
                                                <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                ...
                                <tr>
                                    <td colspan="8" class="px-6 py-12 text-center">
                                        ...
                                        <h3 class="mt-2 text-sm font-medium text-gray-900 dark:text-white">
                                            @if(request('stock_status') == 'low')
                                                Tidak ada produk dengan low stock
                                            @elseif(request('stock_status') == 'out')
                                                Tidak ada produk yang out of stock
                                            @elseif(request('stock_status') == 'in')
                                                Tidak ada produk yang in stock
                                            @else
                                                Tidak ada produk
                                            @endif
                                        </h3>
                                        <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                                            @if(request('stock_status'))
                                                ...
                                                <a href="{{ route('products.index') }}" class="text-indigo-600 hover:text-indigo-500">lihat semua produk</a>
                                            @else
                                                Mulai dengan menambahkan produk baru.
                                            @endif
                                        </p>
                                        @unless(request('stock_status'))
                                            <div class="mt-6">
                                                <a href="{{ route('products.create') }}" class="inline-flex items-center px-4 py-2 bg-indigo-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 dark:focus:ring-offset-gray-800 transition ease-in-out duration-150">
                                                    <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4" />
